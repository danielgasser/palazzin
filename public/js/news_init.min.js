var fillPostContent = function (d, editCheck) {
  if ($('#edit').val() === '1') {
    return '<a id="link_post_' + d.id + '"></a>' + '<div class="row postrow">' + '<div class="col-sm-3 col-md-3 col-xs-12 posts">' + d.created_at + '<br><h4><a href="' + window.urlTo + 'user/profile/' + d.user_id + '">' + d.user_first_name + ' ' + d.user_name + '</a></h4>' + editCheck + '</div>' + '<div class="col-sm-9 col-md-9 col-xs-12 post-entry">' + d.post_text + '</div>' + '</div>';
  }

  return '<div id="post_' + d.id + '">' + '<a id="link_post_' + d.id + '"></a>' + '<div class="row postrow">' + '<div class="col-sm-3 col-md-3 col-xs-12 posts">' + d.created_at + '<br><h4><a href="' + window.urlTo + 'user/profile/' + d.user_id + '">' + d.user_first_name + ' ' + d.user_name + '</a></h4>' + editCheck + '</div>' + '<div class="col-sm-9 col-md-9 col-xs-12 post-entry">' + d.post_text + '</div>' + '</div>' + '</div>';
};
  ...
  $(document).on('click', '#save_new_post', function (e) {
    ...
    $.ajax({
      ...
      success: function (n) {
        ...
        if (auth === data.uid) {
          editCheck = '<div class="tools">' + '<span id="editPost_' + data.id + '" class="glyphicon glyphicon-pencil edit"></span>' + '<span id="deletePost_' + data.id + '" class="glyphicon glyphicon-remove edit"></span>' + '</div>';
        }
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            if (d.hasOwnProperty('success')) {
              ...
              elHeader.addClass('modal-title-info').removeClass('modal-title-warning').html('<h4>Warnung</h4>');
