var V3Reservation = {
    ...
    onHide: function (e) {
        let classList, startString, endString, dates;
    ...
    addBeforeShowDayNew: function (d, otherClanDate, isEdit) {
        ...
        if (d.getTime() >= today.getTime() && window.newAllGuestBeds['freeBeds_' + dateStr + 'uID' + window.autid] === undefined) {
            isEnabled = true;
        }
    ...
    createTimeLine: function (periods) {
        ...
        while (start <= end) {
            ...
            tl.append('<li id="tl-' + monthStr + '" class="graph-unit-container cal-unit-container">' + '<div class="graph-unit-day cal-unit-day">' + GlobalFunctions.showDate(start, 'short') + '</div></li>');
            tt = recursePeriodFinder(periods, 'period_start_new', monthStr, start);
            $('#tl-' + monthStr).addClass(tt[0].clan_code + '-solid').attr('data_clan', tt[0].clan_code).attr('data-period-id', tt[0].id).append('<span style="text-align: center; display: block">' + tt[0].clan_description + '</span>');
    ...
    loopDates: function (startStr, endStr, separator, func, stop, args) {
        let s, start, e, end, checkDateStr;
        if (stop) {
            return false;
        }
    ...
    tooMuchBeds: function (date) {
        ...
        $('#total_res').addClass('alert-danger').removeClass('alert-info');
        ...
        $.ajax({
            ...
            success: function (data) {
                let res = $.parseJSON(data),
                    str = '';
                $.each(res, function () {
                    str += '<li>' + this.reservation_started_at + '-' + this.reservation_ended_at + ':<br>' + '<a href="' + window.urlTo + '/user/profile/' + this.user_id + '">' + this.user_name + '</a><br>' + '<a href="mailto:' + this.user_mail + '">' + this.user_mail + '</a><br>' + '<a href="tel:' + this.user_fon + '">' + this.user_fon + '</a><br>' + '</li>';
                });
                if (str.length === 0) {
