...
jQuery(document).ready(function() {
    ...
    $("#error-wrap").length > 0 && (a = $(".error-field").first().text(), $("." + a).focus());
    jQuery(document).on("click", "#addRight", function(a) {
        a.preventDefault();
        var b = jQuery("#right_id").val(),
            c = jQuery('[name="id"]').val();
        window.rightFromRole(window.role_rights, b, c)
    });
    jQuery(document).on("click", '#confirmDeleteRight', function() {
        var a = jQuery(this).attr("data-role"),
...
jQuery(document).ready(function() {
    ...
    jQuery(document).on("click", '[id^="deleteRole_"]', function() {
        ...
        jQuery("#confirmDeleteRight").attr('data-role', a);
